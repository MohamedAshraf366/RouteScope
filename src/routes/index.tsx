...
function Convert() {
  ...
  return (
    <AppShell title={t("convTitle")} subtitle={t("convSub")}>
      <PageGuide
        what="Convert geospatial files between formats entirely inside your browser: KML, KMZ, GeoJSON, MapInfo MIF/MID and native MapInfo TAB. The converted geometry is previewed on a map so you can confirm it landed in the right place before using it."
        steps={[
          {
            title: "Pick the output format",
            body: "Choose GeoJSON, KML, KMZ or MapInfo TAB in the format cards on the left.",
          },
          {
            title: "Choose your file",
            body: "Click the upload box and select the source file. For a MapInfo TAB set, select all four files (.tab, .map, .dat, .id) together.",
          },
          {
            title: "Download starts automatically",
            body: "The converted file downloads as soon as the conversion finishes, and a confirmation appears under the upload box.",
          },
          {
            title: "Check the preview",
            body: "The result is drawn on the map on the right, with an attributes list for the first 20 features.",
          },
        ]}
        notes={[
          "Nothing is uploaded — every conversion runs locally on your device.",
          "TAB conversion uses a GDAL WebAssembly engine (~40 MB) downloaded the first time you use it.",
        ]}
      />
